...
import { filter, map, mergeMap, sampleTime } from 'rxjs/operators';
...
function createScroller(config) {
    ...
    return attachScrollEvent(options)
        .pipe(mergeMap((ev) => of(calculatePoints(element, resolver))), map((positionStats) => toInfiniteScrollParams(scrollState.lastScrollPosition, positionStats, distance)), map(({ stats, scrollDown }) => {
        updateScrollState(scrollState, stats.scrolled, stats.totalToScroll);
        return { stats, scrollDown };
    }), filter(({ fire, scrollDown, stats: { totalToScroll } }) => shouldTriggerEvents(fire, config.alwaysCallback, isTriggeredScroll(totalToScroll, scrollState, scrollDown))), map(({ scrollDown, stats: { totalToScroll } }) => {
